...
UBTTask_RotateToFaceTarget::UBTTask_RotateToFaceTarget()
{
	...
	bCreateNodeInstance = false;//  node will not be instanced instead use memory block (FRotateToFaceTargetTaskMemory  -> created in HeaderFile) and template shared with all other BT components
...
bool UBTTask_RotateToFaceTarget::HasReachedAnglePrecision(APawn* QueryPawn, AActor* TargetActor) const
{
	...
	const FVector TargetDirection = (TargetActor->GetActorLocation() - QueryPawn->GetActorLocation());

	const float DotProd = FVector::DotProduct(OwnerForward,TargetDirection);
	const float AngleDiff = UKismetMathLibrary::DegAcos(DotProd);
